...
func (h *Handler) LocalRoutes(r chi.Router) {
	...
	r.Get("/api/local/workspaces", h.LocalListWorkspaces)
	r.Post("/api/local/workspaces", h.LocalCreateWorkspace)
	r.Post("/api/local/workspaces/{id}/switch", h.LocalSwitchWorkspace)
